...
void SecureSocket::connect( std::string & server_name )
{
...
void SecureSocket::accept( std::string & server_name )
{
    const auto ret = SSL_accept( ssl_.get() );
    if ( ret == 1 ) {
        if (SSL_get_servername( ssl_.get(), TLSEXT_NAMETYPE_host_name ) != NULL) {
            std::string s(SSL_get_servername( ssl_.get(), TLSEXT_NAMETYPE_host_name ));
            server_name = s;
        } else {
            server_name = "NULL";
        }
